...
class SpikeFeatureExtractor:
    ...
    def extract_from_dataframe(self, data, verbose=False, num_workers=8, should_preprocess=True):
        """
    ...
    def extract_from_row(self, row):
        ...
        if row['label'] == 2:
            ...
            # If the model is 'da', label 1 is for fast spiking, f > 10 Hz
            if self.model == "da":
                if row['f_spiking'] > 10:
                    row['label'] = 1
            ...
            if len(spiking_times) > 0:
                ISIs = np.diff(spiking_times)
                if len(ISIs) > 0:
                    burst_ISIs = np.split(ISIs, np.where(ISIs > (ISIs.max() + ISIs.min()) / 2)[0])[1:-1]

                    if len(burst_ISIs) > 0:
                        row['nbr_spikes_bursting'] = np.nanmean([len(b) for b in burst_ISIs])
                        row['f_intra_bursting'] = np.nanmean([(1000. / b[1:]).mean() if len(b) > 1 else np.nan for b in burst_ISIs])
                        row['duration_bursting'] = np.nanmean([b[1:].sum() if len(b) > 1 else np.nan for b in burst_ISIs])
                        row['f_inter_bursting'] = np.nanmean([1000. / b[0] if len(b) > 0 else np.nan for b in burst_ISIs])
                    else:
